export const fnRequestAPI = async (URL) => {
  ...
  try {
    ...
    if (!request.ok) {
      console.log(request.status); // 200, 404, 500, etc — el número
      console.log(request.statusText); // "OK", "Not Found", "Internal Server Error" — el texto
      console.log(request.ok); // true si está entre 200-299, false si no
...
export const fnUsuarios = async () => {
  try {
    const datos = await fnRequestAPI(URLUsuarios);
    return datos;
  } catch (error) {
    throw new Error(error.message);
  }
...
const URLUniversidadesArgentina = `http://universities.hipolabs.com/search?country=Colombia`;

export const fnUniversidades = async () => {
  try {
    const datos = await fnRequestAPI(URLUniversidadesArgentina);
    return datos;
  } catch (error) {
    throw new Error(error.message);
  }
