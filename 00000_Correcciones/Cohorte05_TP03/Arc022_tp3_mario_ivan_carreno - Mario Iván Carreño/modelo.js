...
export const calcular_Precio_Venta = (importe_Compra, margen_Ganancia) => {
    return importe_Compra + (importe_Compra * margen_Ganancia) / 100;
};
...
export const calcular_Promedio_Notas = (nota1, nota2, nota3) => {
...
export const condicion_Alumno = (nota_Promedio) => {
    let condicion_Alumno;

    if (nota_Promedio >= 0 && nota_Promedio <= 4) {
        condicion_Alumno = "Desaprobado";
    }
    else {
        if (nota_Promedio > 4 && nota_Promedio <= 7) {
            condicion_Alumno = "Aprobado";
        }
        else {
            if (nota_Promedio > 7 && nota_Promedio <= 9) {
                condicion_Alumno = "Muy Bueno";
            }
            else {
                if(nota_Promedio == 10){
                    condicion_Alumno = "Excelente";
                }
            }
        }
    }
...
export const calcular_Tasa = (tipo_Bebida, importe_Base) => {
    switch (tipo_Bebida) {
        case 1: return (importe_Base) * 5 / 1000; 
        case 2: return (importe_Base) * 1 / 1000; 
        case 3: return (importe_Base) * 7 / 1000; 
        case 4: return (importe_Base) * 2 / 1000; 
        case 5: return (importe_Base) * 15 / 1000; 
        case 6: return (importe_Base) * 1 / 1000; 
        default: console.log("INGRESO UN VALOR INCORRECTO"); break;
    }
}
...
export const calcularImporteAgua = (m3_consumidos) => {
    if (m3_consumidos > 0 && m3_consumidos <= 50) {
        return 50 * 350;
    }
    else {
        if (m3_consumidos > 50 && m3_consumidos <= 70) {
            return 50 * 350 + (m3_consumidos - 50) * 555.2;
        }
        else {
            if (m3_consumidos > 70) {
                return (50 * 350) + (20 * 555.2) + ((m3_consumidos-70) * 1552.2);
            }
        }
    }
};
...
export const calcular_Tasa_Subsuelo = (importe_Factura_Agua) => {
    return (importe_Factura_Agua * 3) / 100;
}
...
export const calcular_Tasa_FiscalizacionENRE = (importe_Factura_Agua) => {
    return (importe_Factura_Agua * 1.2) / 100;
};
...
export const calcular_Dosis_Insulina = (nivel_Glucosa_Sangre, peso_Corporal, tipo_Diabetes) => {
    if (tipo_Diabetes == 1) {
        if (nivel_Glucosa_Sangre <= 180) {
            return peso_Corporal * 0.5;
        }
        else {
            return (peso_Corporal * 0.5) + (nivel_Glucosa_Sangre * 0.5);
        }
    }
    else {
        if (nivel_Glucosa_Sangre <= 180) {
            return peso_Corporal * 0.2;
        }
        else {
            return (peso_Corporal * 0.2) + (nivel_Glucosa_Sangre * 0.5);
        }
    }
};
...
export const cantidad_Vocales = (cadena_Texto) => {
    cadena_Texto = cadena_Texto.toUpperCase();
    let contador = 0;
...
export const cantidad_Consonantes = (cadena_Texto) => {
    cadena_Texto = cadena_Texto.toUpperCase();
    let contador = 0;
    for (let i = 0; i < cadena_Texto.length; i++) {
        if (cadena_Texto[i] != 'A' && cadena_Texto[i] != 'E' && cadena_Texto[i] != 'I' && cadena_Texto[i] != 'O' && cadena_Texto[i] != 'U') {
...
export const contieneSS = (palabra) => {
...
export const espacios_Blanco = (texto) => {
    let contador = 0;
...
export const contiene_Numero = (texto) => {
    for (let i = 0; i < texto.length; i++) {
