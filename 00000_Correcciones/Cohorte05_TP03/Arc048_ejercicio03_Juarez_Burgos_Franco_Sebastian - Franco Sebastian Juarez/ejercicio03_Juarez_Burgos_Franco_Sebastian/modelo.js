// Ejercicio 13
//Realizar una arrow function que reciba dos parámetros de entrada. El parámetro 1 será el importe decompra de un producto, el segundo será el margen de ganancia que se aplicará sobre ese producto, la funcióndebe retornar el precio de venta que será igual a aplicarle el % de margen de ganancia. La función debe retornarel precio de venta (sin impuestos).Nota: Debe devolver un número//

export const precioDeVenta = (costo, margen) =>
    costo + (costo * margen / 100);

// Ejercicio 14
//Realizar una arrow function que reciba como parámetro las 3 notas que obtuvo un alumno en losdistintos trabajos prácticos de una materia y que a partir de esas notas obtenga el promedio de las mismasNota: Debe devolver un número//

export const promedioDeTres = (n1, n2, n3) =>
    (n1 + n2 + n3) / 3;
...
export const clasificarNota = (nota) => {
    if (nota <= 4) return "Desaprobado"; //nota es menor o igual que 4
    if (nota <= 7) return "Aprobado"; //nota es menor que o igual 7
    if (nota <= 9) return "Muy Bueno"; //nota es menor o igual 9
    if (nota === 10) return "Excelente"; //si la nota es 10 o igual 10
};
...
export const calcularSobreTasa = (importe, tipo) => {
    let tasa = 1;
...
// Ejercicio 17
//Una empresa de distribución de agua potable "Aguas de Catamarca ECSAPEM" utiliza un esquema defacturación por bloques de consumo, donde el precio por metro cúbico aumenta a medida que el usuario consumemás. Este tipo de sistema se aplica con el objetivo de promover el uso responsable del recurso y penalizar elconsumo excesivo.
...
export const tasaSubsuelo = (importe) =>
    importe * 0.03;
...
export const tasaFiscalizacion = (importe) =>
    importe * 0.012;
...
// Ejercicio 21
//Realizar una arrow function que reciba como parámetro una cadena de texto y que devuelva la cantidad de vocales “mayúsculas y/o minúsculas” que tiene la misma.Debe recorrer la cadena con un ciclo for, desde el primer carácter hasta el último, analizar cada uno de los caracteres y determinar si es una vocal o no. No debe utilizar métodos de strings como replace(), split() o expresiones regulares.
//Nota: Debe devolver un número.
...
// Ejercicio 22
//Realizar una arrow function que reciba como parámetro una cadena de texto y devuelva cuántasconsonantes contiene (mayúsculas o minúsculas). Es decir consideramos consonante a todo carácter que NO SEA VOCAL.
...
export const contieneDosS = (cadena) => {
    let contador = 0;

    for (let i = 0; i < cadena.length; i++) {
        let c = cadena[i].toLowerCase();
        if (c === "s") {
            contador++;
...
export const sinNumeros = (cadena) => {
