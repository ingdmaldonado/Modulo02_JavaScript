...
const noContieneNumeros = (texto) => {
    for (let i = 0; i < texto.length; i++) {
        let c = texto[i];

        // Verificar si es un número entre '0' y '9'
        if (c >= '0' && c <= '9') {
            return false; // Encontró un número → no cumple
        }
    }

    return true; // Si terminó el for sin encontrar números
};