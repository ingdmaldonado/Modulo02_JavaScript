...
export const estadoDelAlmuno = (promedio) => {
  let estado = ``;
...
export const calcularSobreTasa = (importeBase, tipoBebida) => {
  ...
  switch (tipoBebida) {
    case 1: {
      // Agua en envases plasticos = 5%
      calculoSobreTasa = (importeBase * 5) / 1000;
      break;
    }
    case 2: {
      // Agua en envases retornables = 1%
      calculoSobreTasa = (importeBase * 1) / 1000;
      break;
    }
    case 3: {
      // Gaseosa en envases plasticos = 7%
      calculoSobreTasa = (importeBase * 7) / 1000;
      break;
    }
    case 4: {
      // Gaseosa en envases retornables = 2%
      calculoSobreTasa = (importeBase * 2) / 1000;
      break;
    }
    case 5: {
      // Bebidas energeticas = 15%
      calculoSobreTasa = (importeBase * 15) / 1000;
      break;
    }
    default: {
      // Cualquier otra bebida no clasificada = 1%
      calculoSobreTasa = (importeBase * 1) / 1000;
      break;
    }
  }
...
export const calcularImporteAgua = (consumoM3) => {
  //consumoM3 = Number(consumoM3);
  ...
  if (consumoM3 <= 70) {
    ...
    tarifa2 = (consumoM3 - 50) * 555.2;
  }
  if (consumoM3 > 70) {
    tarifa1 = 50 * 350.0;
    tarifa2 = 20 * 555.2;
    tarifa3 = (consumoM3 - 70) * 1552.2;
  }

  importeTotal = tarifa1 + tarifa2 + tarifa3;
...
export const calcularTasaSubsuelo = (importeBase) => {
  const tasa = 3;
  const importeTasa = importeBase * tasa;
...
export const calcularDosisInsulina = (
  nivelGlucosa,
  pesoCorporal,
  tipoDiabetes
) => {
  ...
  if (tipoDiabetes === `Tipo 1`) {
    dosisInsulina = pesoCorporal * 50;
    if (nivelGlucosa > 180) {
      dosisInsulina = dosisInsulina + (nivelGlucosa * 50) / 100;
    }
  } else {
    if (tipoDiabetes === `Tipo 2`) {
      dosisInsulina = pesoCorporal * 20;
      if (nivelGlucosa > 180) {
        dosisInsulina = dosisInsulina + (nivelGlucosa * 50) / 100;
      }
    }
...
Nota: Debe devolver un boolean (true ó false).
*/
export const tresEspacios = (cadenaTexto)=>{
