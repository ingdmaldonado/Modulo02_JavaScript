...
export const calcularImporteAgua = (consumo) => {
  // Si el consumo es menor que 50, se factura por 50 m³
  let m3 = consumo < 50 ? 50 : consumo;
  let importe = 0;

  if (m3 <= 50) {
    // Todo dentro del primer bloque
    importe = 50 * 350;
  } else if (m3 <= 70) {
    // Primer bloque + consumo adicional hasta 70
    importe = (50 * 350) + ((m3 - 50) * 555.20);
  } else {
    // Primer bloque + segundo bloque + excedente
    importe = (50 * 350) + (20 * 555.20) + ((m3 - 70) * 1552.20);
  }
...
//calcula el 3% del importe base - devolverlo como número.//
...
//aqui la Tasa de Fiscalización ENRE equivale al 1,2 % del importe base.//
...
// Función que calcula la dosis de insulina recomendada
export const calcularDosisInsulina = (nivelGlucosa, pesoCorporal, tipoDiabetes) => {
  let dosis = 0;

  if (tipoDiabetes === 1) {
    // Tipo 1: 50% del peso corporal + 50% del nivel de glucosa si > 180
    dosis = pesoCorporal * 0.5;
    if (nivelGlucosa > 180) {
      dosis += nivelGlucosa * 0.5;
    }
  } else if (tipoDiabetes === 2) {
    // Tipo 2: 20% del peso corporal + 50% del nivel de glucosa si > 180
    ...
    throw new Error("Tipo de diabetes no válido. Use 1 o 2.");
  }
...
export const contarVocales = (texto) => {
  ...
  for (let i = 0; i < texto.length; i++) {
    ...
    // Si el carácter actual es una vocal, incrementar contador
...
// Arrow function que cuenta cuántas consonantes hay en una cadena
export const contarConsonantes = (texto) => {
  ...
  for (let i = 0; i < texto.length; i++) {
    ...
    if (
      ...
      // Si es una letra, verificamos que NO sea vocal
      if (
...
// Arrow function que verifica si una palabra tiene al menos dos letras "s" (mayúsculas o minúsculas)
export const contieneDosSS = (palabra) => {
  let contador = 0;
  ...
  for (let i = 0; i < palabra.length; i++) {
    ...
    if (letra === 's' || letra === 'S') {
      contador++;
      // Si ya encontramos 2, podemos devolver true inmediatamente
      if (contador >= 2) {
        return true;
      }
    }
  ...
  // Si terminamos el bucle y no hay al menos 2, devolvemos false
  return false;
};
...
// Arrow function que verifica si una cadena tiene al menos tres espacios en blanco
export const contieneTresEspacios = (texto) => {
  ...
  for (let i = 0; i < texto.length; i++) {
    if (texto[i] === ' ') {
      contador++;
      if (contador === 3) {
        // Interrumpimos el bucle cuando encontramos el tercer espacio
        break;
      }
    }
  }

  // Devolvemos true si se encontraron 3 o más espacios, false si no
  return contador >= 3;
};
...
// Arrow function que verifica si una cadena no contiene ningún dígito (0-9)
export const sinNumeros = (texto) => {
  for (let i = 0; i < texto.length; i++) {
    const caracter = texto[i];
    // Si el carácter es un número entre '0' y '9', devolvemos false inmediatamente
    if (caracter >= '0' && caracter <= '9') {
      return false;
    }
  }

  // Si recorrimos toda la cadena y no encontramos números, devolvemos true
  return true;
};
