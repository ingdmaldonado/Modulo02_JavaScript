...
export const fnCalcularImporteAgua = (consumoTotal) => { // consumo de metros cúbicos
    
    let importeBase = 0;

    if ((consumoTotal >= 0) && (consumoTotal <= 50)) {
        importeBase = 50 * 350;  //  primeros 50 metros a razón de 350 pesos por metro cúbico
    } else {
        if ((consumoTotal >= 51) && (consumoTotal <= 70)) {
            importeBase = (50 * 350) + ((consumoTotal - 50) * 555.20); // 51 hasta el 70, a razón de 555,20 pesos por metro cúbico
        } else {
            if (consumoTotal > 70) {
                importeBase = (50 * 350) + (20 *555.20) + ((consumoTotal - 70) * 1552.20) // todo consumo que supere los 70 metros cúbicos se factura a razón de 1.552,20 pesos por metro cúbico
                //console.log(importeBase,consumoTotal,consumoTotal-70); 
                // en el TP esta mal el importe de consumo de 85m3, 
                // 85 (50 × 350,00) + (20 × 555,20) + (15 × 1.552,20) da $51887
            }
        }
    }
