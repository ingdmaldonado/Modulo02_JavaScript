...
import { fnGuardar } from "./modelo23.js";
import { fnRecuperar } from "./modelo23.js";
...
window.onload = () => {
    ...
    const idMentrosCubicos = document.querySelector("#idMetrosCubicos");
    const idBtnCalcular = document.querySelector("#idBtnCalcular");
    const idResultado = document.querySelector("#idResultado");

    fnRecuperar();

    idMentrosCubicos.oninput = () => {
        estadoAplicacion.metrosCubicos = Number(idMentrosCubicos.value);
        fnGuardar(estadoAplicacion);
    }

    idBtnCalcular.onclick = () => {
        let resultado = fnCalcularImporteAgua(estadoAplicacion.metrosCubicos);
        idResultado.textContent = `El importe a pagar es ${resultado}`;
    };
