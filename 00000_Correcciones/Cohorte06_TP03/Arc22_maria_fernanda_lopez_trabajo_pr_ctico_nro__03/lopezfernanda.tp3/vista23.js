...
window.onload= ()=>{
    ...
    const IdConsumo= document.querySelector ("#IdConsumo");
...
IdConsumo.oninput = ()=>{
    estadoAplicacion.consumo = Number (IdConsumo.value);
};

IdBtnCalcular.onclick = ()=>{
     const consumo = Number(IdConsumo.value);
    console.log("Consumo usado:", consumo);

    const resultado= fncalcularImporteAgua (estadoAplicacion.consumo);
    estadoAplicacion.importeCalculado = resultado;
    console.log (`Resultado de importe base $ ${resultado}`);

    IdResultado.textContent = `El importe base a pagar por el clientes es de $ ${resultado}`;
};
    ...
    IdBtnRecuperar.onclick = ()=>{
        let importeBase= localStorage.getItem ("estadoApp");

        if (importeBase)
        {
            let estadoObjeto= JSON.parse (importeBase);
