...
utiliza un esquema de facturación por bloques de consumo, 
donde el precio por metro cúbico aumenta a medida que el usuario consume 
más. 
Este tipo de sistema se aplica con el objetivo de promover 
el uso responsable del recurso y penalizar el 
consumo excesivo. 

El cálculo del importe base se realiza a partir 
de los metros cúbicos leídos en el medidor del cliente. 
...
Finalmente, todo consumo que supere los 70 metros cúbicos 
se considera dentro del bloque excedente o de castigo, 
y se factura a razón de 1.552,20 pesos por metro cúbico. 

Cabe aclarar que los clientes cuyo consumo mensual 
sea inferior a 50 metros cúbicos igualmente abonan 
...
que reciba como parámetro el total de metros cúbicos leídos 
y devuelva el importe base a pagar por el cliente según el esquema tarifario antes 
descripto. 
...
window.onload = () => {
    ...
    const idCalculoAgua = document.querySelector("#idCalculoAgua");
    const idResultadoApagar = document.querySelector("#idResultadoApagar");
    const idGuardar = document.querySelector("#idGuardar");
    const idRecuperar = document.querySelector("#idRecuperar");

    let datos = fnRecuperar("ejercicio23");

    idMetrosCubicos.oninput = () => {

        estadoAplicacion.metrosCubicos = Number(idMetrosCubicos.value);
        fnGuardar("ejercicio23",estadoAplicacion);

    };

    idCalculoAgua.onclick = () => {

        let totalApagar = fncalcularImporteAgua(estadoAplicacion.metrosCubicos);

        console.log(totalApagar);

        idResultadoApagar.textContent = `- Su cantidad de m3: ${estadoAplicacion.metrosCubicos}
    ...
    /* GUARDAR */
        ...
        idRecuperar.onclick = () => {
            ...
            if (datos) {
                ...
                idMetrosCubicos.value = datos.metrosCubicos;
            }
        };
};