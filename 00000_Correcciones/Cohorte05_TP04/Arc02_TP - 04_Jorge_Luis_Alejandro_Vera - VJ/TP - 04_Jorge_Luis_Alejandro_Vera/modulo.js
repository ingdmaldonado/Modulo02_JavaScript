...
// el foeach que recorre las cohortes y muestra los alumnos

    export const fnRecorrerCohorte = (cohorte) =>
    {
        let resultado = "";
        ...
        cohorte.forEach(alumno => 
        {
            ...
            resultado += `${apellido} ${nombre} \n${dni}\nNota final: ${nota_final}\n------------------\n`;//Acá tenia un problema, al hacer click esta función solo me mostraba el ultimo alumno de la lista asi que me puse a googlear y encontre un operador de adición += y me sirvió para que muestre todos los alumnos y para que se vean separados los datos en linea busqué como hacer salto de linea en js
        });

        return resultado;  
    };

//buscador de alumnos por DNI con find

    export const fnBuscarAlumnoConDNI = (dni) =>
    {
        const cohortes = [...cohorte01, ...cohorte02, ...cohorte03, ...cohorte04];//uno las cohortes en un solo vector para hacer un barrido general con el find

        const alumnoDni = cohortes.find(alumno=> alumno.dni === dni);//defino alumnoDni como el resultado del find de alumnos

        if(alumnoDni)
        {
            const {apellido, nombre, nota_final} = alumnoDni;
        ...
        //si el alumnoDni está, se muestran en el visor sino pone que no existe
