...
export const fnAlumnosCohortesVer1 = (cohorteN)=>{ //La fn recibe el parámetro
    ...
    //aquí declaro la variable y la inicializo vacía
    let salida = "";

    //aquí recorro el vector de datos con el método forEach
    cohorteN.forEach(alumno =>{ 

        //aquí acumulo los alumnos por su atributo apellido y nombre
        salida += `${alumno.apellido} ${alumno.nombre}\n`;

        //aquí muestro los datos solicitados
        console.log(`Apellido y Nombres: ${alumno.apellido} ${alumno.nombre}`); 
    });

    //aquí regreso la variable => salida con los datos acumulados
    return salida;
};
...
export const fnTodasLasCohortes = (cohorteN)=>{
    ...
    cohorteN.forEach(({apellido,nombre,nota_final})=>{
        ...
        salida += `${apellido} ${nombre} ${nota_final}\n`;
...
/* Creo esta función para que recorra todos los vectores y me muestre datos puntuales para el ejercicio
en donde se debe buscar al alumno de acuerdo al número de DNI (nombre, apellido y DNI) */

export const fnAlumnoDni = (cohorteN)=>{

    let salida = "";

    cohorteN.forEach(({apellido,nombre,dni})=>{

        salida += `${apellido} ${nombre} ${dni}\n`;
    });
