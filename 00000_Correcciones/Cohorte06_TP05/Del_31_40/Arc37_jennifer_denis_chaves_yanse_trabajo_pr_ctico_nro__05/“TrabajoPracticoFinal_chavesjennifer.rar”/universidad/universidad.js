...
import { fnRecuperarUniversidades } from "./universidadmodelo";
...
Window.onload =()=>{
    ...
    ideBtnRecuperar.onclik = async ()=> {

        const paisElegido = Number (ideselectPais.value);
        console.log (paisElegido);
        
        const datos = await fnRecuperarUniversidades(ideselectPais);
        console.log (datos);
        console.log (datos[0]);
        ...
        const fila = fnUniversidadestoView (datos);
        console.los  (filas);

        fnRender (fila, idCuerpoDeTabla);
