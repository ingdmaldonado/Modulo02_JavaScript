...
window.onload = () => {
    ...
    const idSeletorPais = document.querySelector("#idSeletorPais");

    const idCuerpoDetabla = document.querySelector("#idCuerpoDetabla");



    /*Aqui estoy haciedno click en el boton recuperar */
    idBtnRecuperar.onclick = async () => {
        /* Aqui saco el pais elegido */
        const paisElegido = Number (idSeletorPais.value);
       ...
       console.log(fila);
        /*Aqui le estoy pasando todas las  universidad y devolviendolas en filas */
...
/* 
  ...
  modelo.js     → habla con el servidor y usa la funcion (api.js)
