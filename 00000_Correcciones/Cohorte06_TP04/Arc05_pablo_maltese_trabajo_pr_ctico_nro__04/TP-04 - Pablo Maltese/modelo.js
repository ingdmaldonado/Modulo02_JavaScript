...
export const fnRecorrerVectorPunto01 = cohorte => {
    cohorte.forEach(cohorte => {
            console.log(`Apellido: ${cohorte.apellido} - Nombres: ${cohorte.nombre}`);
    });
};

export const fnExpandirVectorPunto02 = cohorte => {
    cohorte.forEach( cohorte => {
            console.log(`Apellido: ${cohorte.apellido} - Nombres: ${cohorte.nombre}`);
    });
};

//Funcion para if/else
export const fnCantidadAlumnosAprobadosPunto06_v1 =  cohorte =>
    cohorte.reduce( (acumulador, aprobados) => {

        if( aprobados.nota_final > 5 )
        {
            acumulador++;
        }
...
//Funcion para operador ternario
export const fnCantidadAlumnosAprobadosPunto06_v2 =  cohorte =>
    cohorte.reduce( (acumulador, aprobados) => {
        return acumulador = aprobados.nota_final > 5? acumulador = acumulador + 1 : acumulador;
}, 0);