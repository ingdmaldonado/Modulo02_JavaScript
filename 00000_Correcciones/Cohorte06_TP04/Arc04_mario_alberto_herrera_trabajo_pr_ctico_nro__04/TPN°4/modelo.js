...
export const cohorte04 = [
    ...
    { dni: "56789013", nombre: "Miguel", Vega: "Vega", apellido: "Vega", nota_final: 4 }, 
...
export const listarAlumnosConsola = (cohorte) => {
    cohorte.forEach(({ nombre, apellido }) => {
...
export const obtenerAprobados = (alumnos) => alumnos.filter(({ nota_final }) => nota_final > 5);
export const obtenerDesaprobados = (alumnos) => alumnos.filter(({ nota_final }) => nota_final <= 5);


export const ordenarPorNotaAsc = (alumnos) => [...alumnos].sort((a, b) => a.nota_final - b.nota_final);


export const buscarPorDni = (alumnos, dniBuscado) => alumnos.find(({ dni }) => dni === dniBuscado);


export const contarAprobadosIf = (alumnos) => 
    alumnos.reduce((acumulador, { nota_final }) => {
        if (nota_final > 5) {
            return acumulador + 1;
        } else {
            return acumulador;
...
export const contarAprobadosTernario = (alumnos) => 
    alumnos.reduce((acumulador, { nota_final }) => nota_final > 5 ? acumulador + 1 : acumulador, 0);