...
export const mostrarCohorte = (cohorte) => {
    cohorte.forEach(estudiante => {
        console.log(`DNI: ${estudiante.dni}, Nombre: ${estudiante.nombre} ${estudiante.apellido}`);
    });
};

export const unificarCohortes = (cohorte01, cohorte02, cohorte03, cohorte04) => {
    const cohorteUnificadas = [...cohorte01, ...cohorte02, ...cohorte03, ...cohorte04];
    return cohorteUnificadas;
};

export const alumnosAprobados = (cohorteUnificadas) => {
    const aprobados = cohorteUnificadas.filter(estudiante => estudiante.nota_final > 5);
    return aprobados;
};

export const alumnosDesaprobados = (cohorteUnificadas) => {
    const desaprobados = cohorteUnificadas.filter(estudiante => estudiante.nota_final <= 5);
    return desaprobados;
};

export const alumnosOrdenados = (cohorteUnificadas) => {
    const ordenados = [...cohorteUnificadas].sort((a, b) => a.nota_final - b.nota_final);
    return ordenados;
};

export const buscarAlumnoPorDNI = (cohorteUnificadas, dni) => {
    const alumno = cohorteUnificadas.find(estudiante => estudiante.dni === dni);
    return alumno;
};

export const cantidadAlumnosAprobados = (cohorteUnificadas) => {
    return cohorteUnificadas.reduce((acumulador, estudiante) => {
...
export const cantidadAlumnosAprobadosTernario = (cohorteUnificadas) => {
    return cohorteUnificadas.reduce((acumulador, estudiante) => {
        return estudiante.nota_final > 5 ? acumulador + 1 : acumulador;
    }, 0)
};
