/*Ejercicio Nro. 07: 
Estructuras Condicionales (if/else)
Un comercio que vende bolsas de alimentos para mascotas trabaja y comercializa únicamente con tres
grandes marcas de alimentos para perros lo que le permite trabajar con descuentos muy interesantes para sus
clientes. Las marcas con las que trabaja son:
Catálogo de Productos
Codigo Descripción x unidad
(cantidad >= 5) y (cantidad
<=10)
Cantidad >=
11
1 Dogui x 21 kil.
$
38.000,00 $ 36.000,00 $ 34.000,00
2
Tiernitos x 21
kil
$
31.000,00 $ 29.000,00 $ 27.000,00
3 Dogpro x 21 kil
$
46.000,00 $ 44.000,00 $ 42.000,00
Realice un programa en JavaScript que permita ingresar el código del producto y la cantidad de bolsas a
comprar y que solo determine el importe total a pagar.*/

{

    let codigo = 0;
    codigo = Number(prompt("Ingrese el código del producto (1 = dogui, 2 = tiernitos, 3 = dogpro):"));
    let cantidad = 0;
    cantidad = Number(prompt("Ingrese la cantidad de bolsas:"));
    let precioUnitario = 0;
    let importeTotal = 0;

if (cantidad > 0) {
    // Determinar precio base según código y cantidad
    if (codigo === 1) { // Dogui
        if (cantidad < 5) precioUnitario = 38000;
        else if (cantidad <= 10) precioUnitario = 36000;
        else precioUnitario = 34000;
    } 
    else if (codigo === 2) { // Tiernitos
        if (cantidad < 5) precioUnitario = 31000;
        else if (cantidad <= 10) precioUnitario = 29000;
        else precioUnitario = 27000;
    } 
    else if (codigo === 3) { // Dogpro
        if (cantidad < 5) precioUnitario = 46000;
        else if (cantidad <= 10) precioUnitario = 44000;
        else precioUnitario = 42000;
    } 
    else {
        console.log("Código de producto inválido.");
    }

    // Calcular total
    if (precioUnitario > 0) {
        importeTotal = precioUnitario * cantidad;
        console.log("El importe total a pagar es: $" + importeTotal);
    }
} else {
    console.log("La cantidad debe ser mayor a 0.");
}
}